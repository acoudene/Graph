...
/// </summary>
public class VolumeSqlServerDockerTest : IAsyncLifetime
{
  /// <summary>
  /// Container
  /// </summary>
  private readonly MsSqlContainer _testcontainers = new MsSqlBuilder()
      // WARNING: Don't forget to copy database files into these volumes:
      // docker run -d --rm --name dummy -v SqlServerData:/var/opt/mssql/data mcr.microsoft.com/mssql/server:2022-latest tail -f /dev/null
      // docker cp master.mdf dummy:/var/opt/mssql/data/master.mdf
      // docker cp mastlog.ldf dummy:/var/opt/mssql/data/mastlog.ldf
      // docker stop dummy
      .WithVolumeMount("SqlServerData", "/var/opt/mssql/data")
      .WithVolumeMount("SqlServerLog", "/var/opt/mssql/log")
      .Build();
  ...
  public VolumeSqlServerDockerTest(ITestOutputHelper output)
  {
    _output = output;
